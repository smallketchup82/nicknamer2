...
bot.on('messageCreate', async message => {
	...
	const galaxyfilterproper = GalaxyFilter.map(value => value.toLowerCase());

	const messageparser = message.content.trim().toLowerCase().split(' ');

	if (messageparser.some(r => galaxyfilterproper.indexOf(r) >= 0) && message.guild.id === '204965774618656769') return;
